...
CognitoStrategy.prototype.authenticate = function(req, options) {
  ...
  if (options.body) {
    username = req.body[options.body.username || 'username'];
    password = req.body[options.body.password || 'password'];
  } else if (options.header) {
    username = req.get(options.header.username || 'x-auth-username');
    password = req.get(options.header.password || 'x-auth-password');
  } else if (options.basic) {
    try {
      const auth = new Buffer(((req.headers.authorization || '').split(' ')[1] || ''), 'base64').toString().split(':');
      username = auth[0];
      password = auth[1];
    } catch(err) {
      console.error(err);
